...
// jose cần Uint8Array cho secret
...
const { jwtVerify } = await import("jose");
  ...
  await userRepository.sendMail({
    ...
    html: `
      <div style="max-width:600px;margin:0 auto;padding:20px;border:1px solid #e0e0e0;border-radius:8px;font-family:sans-serif;background-color:#ffffff;">
        ...
        <p style="font-size:16px;color:#333;">Hoặc sử dụng mã xác nhận sau:</p>
        <div style="text-align:center;font-size:20px;font-weight:bold;color:#005b96;margin:10px 0;">
          ${OTP}
        </div>
        <p style="font-size:16px;color:#333;">Nếu bạn không thực hiện đăng ký, vui lòng bỏ qua email này.</p>
  ...
  const token = await new SignJWT({
    ...
    .setIssuedAt()
    .setExpirationTime("10y") // jose không hỗ trợ "9999 years", set khoảng 10 năm
    .sign(secretKey);
