  ...
  checkout: async (req, res) => {
    try {
      ...
      if (fullOrder.user?.email) {
        const orderDate = moment(order.created_at); // ngày đặt hàng từ DB
        const day = orderDate.format("DD");
        const month = orderDate.format("MM");

        // Subject theo format: TERA + ngày + tháng + ID đơn hàng
        const customSubject = `Đơn hàng TERA${month}${day}${order.orders_id} của bạn đã được xác nhận`;
